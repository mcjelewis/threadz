...
if ($output_format eq "svg") {
	...
	print header(-type=>"image/svg+xml",
		     -attachment=>"Threadz-visualization.svg");
	print $data;
	exit(0);
}
## PDF/PNG output
	...
	my (undef, $input_file) = tempfile("Threadz-visualization.svg.XXXXXXX", OPEN=>0, TMPDIR=>1, UNLINK=>1);
	my (undef, $output_file) = tempfile("Threadz-visualization.out.XXXXXXX", OPEN=>0, TMPDIR=>1, UNLINK=>1);
	...
	print header(-type=>$mime_type,
		     -attachment=>"Threadz-visualization.$output_format");
	print $pdf_data;
